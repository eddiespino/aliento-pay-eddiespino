import type { DelegatorInfo, OptimizedDelegationsResult } from '../lib/get-delegations';
...
export class PaymentCalculatorService {
  ...
  public calculatePayments(
    delegationsResult: OptimizedDelegationsResult,
  ...
  private calculateIndividualPayments(
    ...
    for (const delegation of delegations) {
      const percentage = (delegation.currentHP / totalDelegationsHP) * 100;
      const paymentAmount = (delegation.currentHP / totalDelegationsHP) * totalAmount;

      // Solo incluir pagos que sean mayores al mínimo
      if (paymentAmount >= this.MIN_PAYMENT_AMOUNT) {
        payments.push({
          delegator: delegation.delegator,
          delegatedHP: this.roundToPrecision(delegation.currentHP),
  ...
  private validateCalculationParams(params: PaymentCalculationParams): void {
    ...
    if (
      !Number.isFinite(params.interestPercentage) ||
      !Number.isFinite(params.hivePowerToConsider)
    ) {
  ...
  public validatePayments(result: PaymentCalculationResult): ValidationResult {
    ...
    if (result.summary.percentageDistributed < 95) {
      issues.push(
        `Solo se distribuirá el ${result.summary.percentageDistributed}% del monto total`
      );
    }
